...
1. การวิเคราะห์แนวเรื่อง — สำคัญที่สุด:
   - ให้ยึด "ชื่อเรื่อง" และ "รูปต้นฉบับ" (ถ้ามี) เป็นหลัก อย่าเชื่อ niche ของเพจเกินไป
     (เพจแนว "ซีรีส์จีน" ไม่ได้แปลว่าทุกคลิปเป็นจีน · บางคลิปเป็นฝรั่ง/เกาหลี/ญี่ปุ่น)
   - ถ้าชื่อเรื่องมีคำว่า "ฝรั่ง / ฮอลลีวูด / อเมริกัน / ยุโรป / Western" → สไตล์โปสเตอร์ฮอลลีวูดฟอร์มยักษ์ (ห้ามจีน)
   - ถ้าชื่อเรื่องมีคำว่า "เกาหลี / K-drama / KDrama / โอปป้า" → สไตล์ซีรีส์เกาหลีโรแมนติก (ห้ามจีน)
   ...
   - ถ้าชื่อเรื่องมีคำว่า "จีน / ซีรีส์จีน / เทพเซียน / กำลังภายใน / ย้อนยุค" → สไตล์จีนโบราณ
...
function detectGenreFromTitle(title) {
    ...
    if (/ฝรั่ง|hollywood|western|american|british|europ|ยุโรป|อเมริก|อังกฤษ/i.test(t)) {
        return { genre: 'western', th: 'ซีรีส์ฝรั่ง / ฮอลลีวูด', style: 'โปสเตอร์ฮอลลีวูดฟอร์มยักษ์ ตัวละครคอเคเซียน' };
    }
    // Korean
    if (/เกาหลี|korean|\bk-?drama\b|โอปป้า|oppa/i.test(t)) {
        return { genre: 'korean', th: 'ซีรีส์เกาหลี', style: 'ซีรีส์เกาหลีโรแมนติก ตัวละครเอเชียเกาหลี' };
    }
    // Japanese
    if (/ญี่ปุ่น|japanese|\bjapan\b|\bjdrama\b|อนิเมะ|anime/i.test(t)) {
        return { genre: 'japanese', th: 'หนัง/ซีรีส์ญี่ปุ่น', style: 'สไตล์ญี่ปุ่น ตัวละครเอเชียญี่ปุ่น' };
    }
    // Thai
    ...
    // but also so "ซีรีส์ฝรั่ง" in a cn-configured page isn't misread as chinese.
    if (/จีน|chinese|\bcdrama\b|เทพเซียน|กำลังภายใน|ย้อนยุค|หวงเอ้อ|ฮ่องเต้|บู๊เฮี๊ยบ/i.test(t)) {
        return { genre: 'chinese', th: 'ซีรีส์จีน', style: 'สไตล์จีน ตัวละครเอเชียจีน' };
    }
    return null;
}
...
class CoverService {
    ...
    _buildPrompt({ videoTitle, niche, clipIndex, totalClips, pageOverridePrompt, searchKeyword, referenceDescription }) {
        ...
        // overrides the page niche — prevents the "page is 'ซีรีส์จีน' but this clip is
...
2. แนวเรื่อง: ซีรีส์จีนย้อนยุค / ซีรีส์จีนปัจจุบัน / ฝรั่งฮอลลีวูด / เกาหลีโรแมนติก / ญี่ปุ่น / ไทย / อื่นๆ
...
ห้ามอธิบายว่ารูปเป็นปก/โปสเตอร์ — ให้บอกเฉพาะว่า "เนื้อเรื่อง" เป็นแนวอะไร` },
    ...
    // OpenAI image models. DALL·E 3 and gpt-image-1 share the endpoint but need DIFFERENT bodies:
    // gpt-image-1 rejects DALL·E's size '1024x1792', quality 'standard', and the response_format
    // param outright (HTTP 400). It only accepts 1024x1024 / 1024x1536 / 1536x1024 / auto, quality
    // low|medium|high|auto, and always returns b64_json (no response_format).
    async _genOpenAI(apiKey, model, prompt) {
        ...
        if (model === 'gpt-image-1') {
            body = {
                ...
                size: '1024x1536',   // portrait, closest gpt-image-1 option to 9:16
                quality: 'high',
                n: 1
            };
        } else {
    ...
    async generateCover({ videoPath, videoTitle, niche, clipIndex, totalClips, pageOverridePrompt,
        ...
        try {
            ...
            // Must clear 20KB — orchestrator.js (977/1252/1269) rejects any cover < 20KB and flips
            // the job to FAILED, so a 10KB "success" here would be thrown away downstream anyway.
            if (fs.existsSync(outPath) && fs.statSync(outPath).size >= 20 * 1024) {
