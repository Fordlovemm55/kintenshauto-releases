...
// NAV grouped into sections for beginner clarity. The injected watcher
// nav-item (watcher-injection.js) appears naturally after 'home' — it adds
// its own nav-item with a far more complete UI (channel CRUD,
// pending-approvals approve-all, AI caption per-channel, advanced filters).
const NAV_SECTIONS = [
  {
    section: 'ใช้ประจำ',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'comments', icon: 'icon-comments', th: 'คอมเมนต์อัตโนมัติ' },
      { key: 'ai',       icon: 'icon-ai',       th: 'แคปชั่น AI' }
    ]
  },
  {
    section: 'ระบบ',
...
export default function Dashboard({ user }) {
  ...
  const goToWatcher = useCallback(() => {
    ...
    if (item) {
      ...
      showToast('กำลังเปิดหน้าตามช่อง...', 'ถ้าไม่ขึ้น ลองกดเมนูด้านซ้าย', 'info');
    }
      ...
      try {
        ...
        sock.on('auth:kicked', (payload) => {
          ...
          const msg = payload?.reason === 'user_banned'
            ? 'บัญชีของคุณถูกระงับ กำลังกลับสู่หน้าเข้าสู่ระบบ'
            : 'มีการเข้าสู่ระบบจากอุปกรณ์อื่น กำลังกลับสู่หน้าเข้าสู่ระบบ';
          alert(msg);
  ...
  const startPipeline = async () => {
    if (!selectedPage) { showToast('เลือกเพจก่อน', 'ต้องเพิ่มบัญชี + เพจก่อนเริ่มโพสต์', 'warning'); return; }
    if (!keyword.trim()) { showToast('ใส่คำค้น', 'ต้องมีคำค้นสำหรับค้นคลิป', 'warning'); return; }
    setRunning(true);
    try {
      ...
      showToast('เริ่มแล้ว', 'ระบบกำลังทำงาน ติดตามได้ที่คิวงาน', 'success');
      refresh();
    } catch (e) {
  ...
  return (
    <div className="app-shell">
      <SamuraiBackground variant="app" opacity={0.28} />
      ...
      <nav className={`app-sidebar${sidebarOpen ? ' open' : ''}`} aria-label="เมนูหลัก">
        ...
        <div style={{ marginTop: 24, padding: '0 14px' }}>
          <div className="jp-divider">ทางลัด</div>
          <button className="btn-ghost" style={{ width: '100%', fontSize: 12, padding: '8px 10px' }}
                  onClick={() => window.kintenshauto?.openLogs?.()}>
            เปิดโฟลเดอร์บันทึก
          </button>
        </div>

        <div style={{ marginTop: 'auto', padding: '14px' }}>
          <div className="jp-divider">ออกจากระบบ</div>
          {user?.email && (
          ...
          <button
            className="btn-ghost"
            style={{ width: '100%', fontSize: 12, padding: '8px 10px', color: 'var(--danger)' }}
            onClick={async () => {
      ...
      <main className="app-main">
        ...
        {nav === 'localclips' && <LocalClipsView showToast={showToast} />}
        {nav === 'comments' && <CommentsView showToast={showToast} />}
        {nav === 'ai' && <AICaptionsView showToast={showToast} />}
        {/* nav === 'profiles' renders nothing here — the injected profiles
            overlay paints over .app-main. */}
      </main>
    ...
    <div className="fade-in">
      <div className="home-stat-grid" style={{ marginBottom: 20 }}>
        ...
        <div
          ...
          aria-label={`รออนุมัติ ${stats.pending_approvals} คลิป — ไปหน้าตามช่องอัตโนมัติ`}
        >
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        <div className="panel-header">
          <div>
            <div className="panel-title">เริ่มต้นใช้งาน (สำหรับมือใหม่)</div>
          </div>
        </div>
        <ol style={{ margin: 0, paddingLeft: 20, lineHeight: 1.9, fontSize: 14 }}>
          <li>เพิ่มบัญชีและเพจที่จะโพสต์ (เมนูจัดการบัญชี)</li>
          <li>เพิ่มช่องที่อยากตามดู (เมนูตามช่องอัตโนมัติ)</li>
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            ...
            <div className="panel-subtitle">เลือกเพจ ใส่คำค้น แล้วกดเริ่ม — ระบบจะค้น ตัด โพสต์ให้อัตโนมัติ</div>
          </div>
        </div>
        ...
        {pages.length === 0 ? (
          ...
          <div className="home-quick-grid">
            ...
            <div>
              <label htmlFor="qp-kw">คำค้นหาคลิป</label>
              <input id="qp-kw" placeholder="เช่น ซีรีส์จีน, ซีรีส์เกาหลี"
                     value={keyword} onChange={e => setKeyword(e.target.value)}/>
            </div>
            <button className="btn-primary" onClick={startPipeline} disabled={running}
                    style={{ padding: '10px 24px', whiteSpace: 'nowrap' }}>
              {running ? 'กำลังเริ่ม...' : 'เริ่มโพสต์'}
            </button>
          </div>
        )}
      </div>
          ...
          <div>
            {recentJobs.map(j => (
              <div key={j.id} style={{
                ...
                <div>
                  <div style={{ fontSize: 14 }}>{j.video_title || 'คลิป'} · คลิป {j.clip_index}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{j.page_name} · {j.scheduled_at}</div>
                </div>
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        ...
        <button className="btn-ghost" onClick={goToWatcher} style={{ fontSize: 12 }}>
          เปิดหน้าตามช่อง →
        </button>
      </div>
      {channels.length === 0 ? (
